import { stripe } from "@/app/payment_gateway/stripe";
...
export async function POST(req: Request) {
  ...
  const success_url = !body.customerId
    ? `${origin}/auth/signin?session_id={CHECKOUT_SESSION_ID}`
    : `${origin}/auth/signup?session_id={CHECKOUT_SESSION_ID}`;
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${origin}/auth/signup?session_id={CHECKOUT_SESSION_ID}`,
    });
