...
const BookingPage: React.FC = () => {
  ...
  const [canceledCount, setCanceledCount] = useState(0);
  const [rentalData, setRentalData] = useState<{ [key: string]: RentData }>({});
  const [vehicledData, setVehicledData] = useState<{
    ...
    const applyFilters = (bookingsData: BookingData[]) => {
      const filteredBookings = bookingsData.filter((booking) => {
        ...
        const isVehicleMatch =
          vehicleFilter === "all" ||
          (vehicleDataForBooking &&
            vehicleDataForBooking.brand === vehicleFilter);

        const isDateMatch =
          !dateFilter ||
          (rentalDataForBooking &&
            new Date(rentalDataForBooking.StartDate) <= new Date(dateFilter) &&
            new Date(rentalDataForBooking.EndDate) >= new Date(dateFilter));
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <main className="mt-10">
          <section className="mb-8 rounded-lg bg-white p-6 shadow dark:border-strokedark dark:bg-boxdark">
            <h2 className="mb-4 text-2xl font-bold text-meta-5">Overview</h2>
            <div className="flex flex-wrap justify-around text-center">
            ...
            <div className="flex flex-wrap gap-4">
              <div>
                <select
                  ...
                  className="flex-grow rounded border p-2 md:flex-none"
                >
                  <option value="all">All</option>
                  <option value="pending">Pending</option>
                  <option value="confirmed">Confirmed</option>
              ...
              <div>
                ...
                >
                  <option value="all">All</option>
                  {Object.values(vehicledData).map((vehicle) => (
                    <option key={vehicle.name} value={vehicle.brand}>
                      {vehicle.brand}
                    </option>
                  ))}
                </select>
